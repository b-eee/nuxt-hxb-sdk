        ...
        <el-collapse model-value="1">
          <el-collapse-item name="1">
            ...
            <el-space direction="vertical" :size="4" v-if="curAppId">
              <el-select
                v-model="curDatastoreId"
                class="m-2"
                placeholder="Select"
                size="large"
                @change="onSelectDatastore"
              >
                <el-option
...
import { ApplicationAndDataStore } from "@hexabase/hexabase-js/dist/lib/types/project";
...
export default defineComponent({
  ...
  methods: {
    ...
    onSelectDatastore() {
      this.$router.push({
        ...
        query: {
          datastore_id: this.curDatastoreId,
          application_id: this.curAppId,
        },
      });
  ...
  computed: {
    getCurAppIndex() {
      const idx = this.appDatastore.findIndex(
        (el: any) => el.application_id === this.curAppId
      );
      return idx > 0 ? idx : 0;
    },
  },
